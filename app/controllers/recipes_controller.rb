class RecipesController < ApplicationController
  ...
  def create
    @recipe = Recipe.new(recipe_params.except(:new_ingredient_names, :new_ingredient_name))
    @recipe.chef_id = current_chef.id

    if @recipe.save
      names = params.dig(:recipe, :new_ingredient_names) || [ params.dig(:recipe, :new_ingredient_name) ]
      names.each do |n|
        next if n.blank?
        name = n.to_s.strip
  ...
  def update
    if @recipe.update(recipe_params.except(:new_ingredient_names, :new_ingredient_name))
      names = params.dig(:recipe, :new_ingredient_names) || [ params.dig(:recipe, :new_ingredient_name) ]
      names.each do |n|
        next if n.blank?
        name = n.to_s.strip
  ...
  def recipe_params
    params.require(:recipe).permit(:name, :description, :chef_id, :new_ingredient_name, new_ingredient_names: [], ingredient_ids: [])
  end
